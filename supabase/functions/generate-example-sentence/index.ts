...
serve(async (req) => {
  ...
  try {
    ...
    const { verb, preposition, meaning } = validation.data;
    ...
    console.log(`Generating example for: ${verb} ${preposition} - ${meaning}`);
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an English teacher. Generate a simple, natural example sentence using the given phrasal verb. Then provide its Spanish translation. Format: English sentence|Spanish translation'
          },
          {
            role: 'user',
            content: `Create an example sentence using the phrasal verb "${verb} ${preposition}" which means "${meaning}".`
          }
        ],
      }),
    });
